		...
		<div class="card-body">
			<div class="d-flex justify-content-between align-items-center mb-3">
				...
				<div class="d-flex gap-2 align-items-center">
					<input id="table-search" type="search" class="form-control form-control-sm" placeholder="Pesquisar..." style="min-width:200px;">
				...
				<table class="table table-hover align-middle">
					...
					<tbody id="users-table-body">
						...
						<tr class="user-row">
							...
							<td class="user-name">{{ $user->client->name_and_surname ?? $user->name ?? '-' }}</td>
							...
							<td class="text-end">
								<a href="" class="btn btn-sm btn-light border" title="Ver"><i class="bi bi-eye"></i></a>
								<a href="{{ route('user.edit', $user) }}" class="btn btn-sm btn-outline-primary" title="Editar"><i class="bi bi-pencil"></i></a>
								<button type="button" class="btn btn-sm btn-outline-danger btn-confirm" title="Excluir"
		...
		(function(){
			...
			input.addEventListener('input', function(){
				...
				rows.forEach(r => {
					...
					const match = q === '' || name.includes(q) || email.includes(q) || profile.includes(q);
